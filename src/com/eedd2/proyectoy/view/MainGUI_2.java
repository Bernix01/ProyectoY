...
public class MainGUI_2 {
	...
	public static void main(String... args) {
		...
		//obtener todas las posibles peliculas que contengan esa palabra, ignorando mayusculas y minusculas
		List<Pelicula> coincidencias = orbe.vertexSet().stream()
				.filter(pelicula -> pelicula.getNombre().toLowerCase().contains(palabra.toLowerCase())).collect(Collectors.toList());
		...
		//por cada coincidencia, se ejecuta una busqueda en anchura y se almacena en un set
		coincidencias.forEach(peli ->{

			//porque los colores son geniales, darle un color especifico a todas las coincidencias de palabra
		...
		//porque los colores son geniales, darle un color especifico a la pelicula buscada
		prototipo.setColor(Pelicula.COLOR.BUSCADO);

		//creamos un subgrafo con el set de peliculas que resultaron de las busquedas en anchura
