...
public class ProyectoY {
	...
	public static void mostrar(AbstractGraph<Pelicula, ProyectoYEdge> grafo) throws IOException, URISyntaxException {
		...
		//Mover el nodo buscado al final para que aparezca sobre los demas
		List<Pelicula> nodos = grafo.vertexSet().stream().collect(Collectors.toList());
		Pelicula buscado = nodos.parallelStream()
				.filter(pelicula -> (pelicula.getColor() != null) && pelicula.getColor().equals(Pelicula.COLOR.BUSCADO))
				.findFirst().get();
		nodos = nodos.parallelStream().filter(pelicula -> (pelicula.getColor() == null) || ((pelicula.getColor() != null) && !pelicula.getColor().equals(Pelicula.COLOR.BUSCADO)))
				.collect(Collectors.toList());
		nodos.add(buscado);
